...
//If the POST method is used, this information is sent to your program using standard input.
	if ((pid = fork()) == 0)
	{
		...
		if (clearenv() != 0
			...
			|| setenv("HTTP_REFERER", request.getHeaderValue("Referer"), 1) != 0
			|| setenv("HTTP_USER_AGENT", request.getHeaderValue("User-Agent"), 1) != 0
			|| setenv("PATH_INFO", , 1) != 0
